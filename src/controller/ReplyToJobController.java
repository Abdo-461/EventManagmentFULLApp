...
public class ReplyToJobController {
	
	/*Labels on the scene - START*/
	@FXML
	...
	/*Labels on the scene - END*/
	
	//instance variables to capture the reply info
	String replyId;
	String postID;
	double replyValue;
	String responderID;
	
	//reply function to job posts
	@FXML
	void replyToJob() {
	...
	void databaseInsert() {
		...
		final String TABLE_NAME_Reply = "REPLYjob";
		...
		try (Connection con = UniLinkDB.getConnection(DB_NAME);
			...
			{
			@SuppressWarnings("unused")
			Reply replyJ = this.createReplyObj(replyId,postID,replyValue, responderID);
